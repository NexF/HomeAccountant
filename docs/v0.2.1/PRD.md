...
v0.2.1 引入 **MCP 接口层**，将 v0.2.0 已有的后端 API 封装为 MCP Tools，使 AI Agent（Claude Desktop、Cursor、自定义 Agent 等）能够直接调用咕咕记账系统的能力。同时新增**分录类型转换**功能，允许用户在不删除重建的情况下将已有分录从一种类型转换为另一种类型。
...
**MCP Server 是 AI Agent 与咕咕记账系统之间的桥梁。** 它不引入新的业务逻辑，而是将已有的 REST API 封装为 LLM 友好的 Tool 接口。
...
| 配置项 | 环境变量 | 说明 | 默认值 |
|--------|---------|------|--------|
| Server 基础 URL | `HA_SERVER_URL` | 咕咕记账 Server 的地址 | `http://localhost:8000` |
...
| 网络错误 | `无法连接到咕咕记账服务器 {url}，请检查服务是否运行` |
